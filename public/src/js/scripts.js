...
function applyContent(data) {
    for (const [key, item] of Object.entries(data)) {
        ...
        // ── DYNAMIC LISTS ────────────────────────────────────────────────────────
        if (item.type === 'list_object' && key === 'skills_list') {
            renderSkillsList(value);
            continue;
        }
...
function renderSkillsList(skillsArr) {
    const wrapper = document.getElementById('skills-cards-wrapper');
    if (!wrapper || !skillsArr) return;
    ...
    wrapper.innerHTML = skillsArr.map((card, i) => `
        <div class="video-card"${skillsArr.length === 1 ? ' style="max-width: 600px; margin: 0 auto;"' : ''}>
            ...
            <a href="${escapeHtml(card.btn_link)}" class="btn btn-primary">JOIN NOW</a>
        </div>
    `).join('');
}
