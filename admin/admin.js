...
const SECTION_KEYS = {
    ...
    support: ['support_email', 'support_phone', 'support_response_time', 'support_hours', 'support_intro']
};
...
function populateFields(data) {
    ...
    if (data.skills_list) buildSkillsCards(data.skills_list.value || []);
    ...
    <div class="review-item">
      ...
      <div class="fields-grid">
        <div class="field-group field-full">
          <label class="field-label">Review Video URL</label>
          <div style="margin-top:0px;">
            <label class="field-label" style="font-size:0.8rem;">Paste video URL (e.g. Cloudinary, YouTube embed, etc.)</label>
          ...
          <p style="font-size:0.75rem;color:var(--text-muted);margin-top:4px;">Current URL: <span id="video_reviews_list_${i}_video-current" style="color:var(--accent);word-break:break-all;">${rev.video || 'none'}</span></p>
        </div>
        <div class="field-group field-full">
...
// ─── DYNAMIC: BUILD SKILLS CARDS ─────────────────────────────────────────────
function buildSkillsCards(cardsArr) {
    const container = document.getElementById('skills-cards-container');
    if (!container) return;
    if (!cardsArr || cardsArr.length === 0) cardsArr = [{ video: '', text: '', btn_link: '' }];

    // Store in contentData
    if (!contentData['skills_list']) {
        contentData['skills_list'] = { value: cardsArr, type: 'list_object' };
    } else {
        contentData['skills_list'].value = cardsArr;
    }

    container.innerHTML = cardsArr.map((card, i) => `
    <div class="review-item">
      <div class="review-item-header">
        <div>
        ...
        ${cardsArr.length > 1 ? `<button class="btn" style="background:var(--danger);padding:4px 8px;font-size:12px" onclick="removeSkillCard(${i})">Remove</button>` : ''}
      </div>
      <div class="fields-grid">
        <div class="field-group field-full">
          <label class="field-label">Video URL</label>
          <div style="margin-top:8px;">
            <label class="field-label" style="font-size:0.8rem;">Paste Cloudinary video URL</label>
            <div style="display:flex;gap:8px;margin-bottom:8px;">
              <input type="text" id="skills_card_${i}_video" placeholder="https://res.cloudinary.com/.../video/upload/..." value="${card.video || ''}" style="flex:1;">
            </div>
          </div>
          <p style="font-size:0.75rem;color:var(--text-muted);margin-top:4px;">Current URL: <span id="skills_card_${i}_video_current" style="color:var(--accent);word-break:break-all;">${card.video || 'Not set'}</span></p>
        </div>
        <div class="field-group field-full">
          <label class="field-label">Card ${i + 1} Description</label>
          <textarea id="skills_card_${i}_text" rows="2" placeholder="Card description...">${card.text || ''}</textarea>
        </div>
        <div class="field-group">
          <label class="field-label">Card ${i + 1} Button Link</label>
          <input type="text" id="skills_card_${i}_btn" placeholder="#vip-pricing" value="${card.btn_link || ''}">
        </div>
      </div>
    </div>
  `).join('');

    // Show/hide Add button (max 2)
    const addBtn = document.getElementById('addSkillCardBtn');
    if (addBtn) addBtn.style.display = cardsArr.length < 2 ? 'inline-flex' : 'none';
}

function addSkillCard() {
    const list = contentData['skills_list'] ? contentData['skills_list'].value : [];
    if (list.length >= 2) {
        showToast('Maximum 2 skill cards allowed', 'error');
        return;
    }
    list.push({ video: '', text: '', btn_link: '' });
    buildSkillsCards(list);
    showToast('New skill card added. Fill in the details and click Save.', 'success');
}

function removeSkillCard(index) {
    const list = contentData['skills_list'] ? contentData['skills_list'].value : [];
    if (list.length <= 1) {
        showToast('Minimum 1 skill card is required', 'error');
        return;
    }
    if (!confirm(`Remove Skill Card ${index + 1}? Click Save to confirm.`)) return;
    list.splice(index, 1);
    buildSkillsCards(list);
    showToast('Skill card removed. Click Save Skills to apply.', 'success');
}

async function saveSkillCards() {
    // Read current values from the DOM inputs
    const list = contentData['skills_list'] ? contentData['skills_list'].value : [];
    const updatedList = list.map((card, i) => ({
        video: (document.getElementById(`skills_card_${i}_video`) || {}).value?.trim() || '',
        text: (document.getElementById(`skills_card_${i}_text`) || {}).value?.trim() || '',
        btn_link: (document.getElementById(`skills_card_${i}_btn`) || {}).value?.trim() || ''
    }));

    contentData['skills_list'].value = updatedList;
    ...
    try {
        const res = await fetch('/api/content/bulk/update', {
            ...
            body: JSON.stringify({ updates: { skills_list: updatedList } })
        });
        ...
        const data = await res.json();
        if (data.success) {
            showToast(`✅ Skills saved! ${updatedList.length} card(s) updated.`, 'success');
            buildSkillsCards(updatedList);
        } else {
            showToast(data.error || 'Save failed', 'error');
        }
    } catch (err) {
